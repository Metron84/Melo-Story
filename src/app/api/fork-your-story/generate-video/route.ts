...
export async function POST(request: NextRequest) {
  try {
    ...
    // Production mode - generate real videos via Replicate/Kling
    // Demo mode removed - all videos are generated live

    // Generate video using Replicate/Kling
    let videoResult;
    try {
      ...
      if (errorMessage.includes('API') || errorMessage.includes('auth') || errorMessage.includes('token')) {
        return NextResponse.json(
          { error: 'Invalid video generation API key. Please check your REPLICATE_API_TOKEN configuration.' },
          { status: 500 }
        );
      }
