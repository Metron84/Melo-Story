 ...
 */
// Type guard for FileOutput with url() method
...
function isFileOutput(obj: unknown): obj is FileOutput {
  return (
    ...
    obj !== null &&
    'url' in obj &&
    typeof (obj as { url: unknown }).url === 'function'
  );
}
...
export async function generateVideoTrailer(
  ...
  const { prompt, duration = 5, startImage } = options;
  ...
  try {
    ...
    // Replicate returns a FileOutput object with .url() method or string/array
    let videoUrl: string;
    ...
    if (isFileOutput(output)) {
      ...
      } else if (isFileOutput(firstOutput)) {
